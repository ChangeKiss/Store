...
public class AnnularView extends View{
     ...
     * #f72d2d
     */
    private int mRingColor = Color.parseColor("#f72d2d");
    /**
     * 第二圈的颜色
     * #6495ED
     */
    private int mSectorColor = Color.parseColor("#6495ED");
    /**
     * 终点扇形角度
     */
    ...
    public void setPeopleBonus(int peopleBonus) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        getPer(mSweepAngle, 360);  //设置百分百
        drawText(canvas, progressText + teamText(getTeamBonus()), textX1, textY1, mTextPaint);   //绘制文字
        // 获取进度圆弧的中心点
        ...
        getPer(360 - mSweepAngle, 360);  //设置百分百
        drawText(canvas, reminderText + peopleText(getPeopleBonus()), textX2, textY2, mTextPaint);  //绘制文字
    }
    ...
    private String getPer(float now, float total) {
        ...
        String result = numberFormat.format(now / total);
        return result;
    }
     ...
     * 设置个人分红保留两位小数
     */
    private String peopleText(int peopleBonus){
        return String.format("%.2f", peopleBonus / 100.00);
    }

    /**
     * 设置团队分红保留两位小数
     */
    private String teamText(int teamBonus){
        return String.format("%.2f", teamBonus / 100.00);
    }
